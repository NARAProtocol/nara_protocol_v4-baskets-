...
contract ForkBuyProof is Test {
    ...
    function setUp() public {
        ...
        if (manager == address(0) || v3Adapter == address(0) || aeroAdapter == address(0) || naraToken == address(0)) {
    ...
    function testForkBuyCore() public {
        ...
        NARAImmutableBasketPositionManagerV1.BuyParams memory p = NARAImmutableBasketPositionManagerV1.BuyParams({
            paymentToken: USDC,
            inputAmount: input,
            directAmountsIn: new uint256[](5),
        ...
        (uint256 tokenId, uint256[] memory bought) = NARAImmutableBasketPositionManagerV1(manager).buyBasket(p);
        vm.stopPrank();
        ...
        for (uint256 i; i < 5; i++) {
            assertGt(bought[i], 0, "each asset bought > 0");
        }
        assertEq(IERC20(USDC).balanceOf(user), 0, "all USDC spent");
    }

    function _m(uint256 q) internal pure returns (uint256) {
        return q * 9950 / 10000;
    }
        ...
        returns (NARAImmutableBasketPositionManagerV1.SwapInstruction memory)
    {
        return NARAImmutableBasketPositionManagerV1.SwapInstruction(
            v3Adapter, USDC, o, amt, mo, abi.encode(f, uint160(0))
        );
    }
