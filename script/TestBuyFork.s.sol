...
contract TestBuyFork is Script {
    ...
    function run() external {
        ...
        minOut[0] = _slip(qLink);
        minOut[1] = _slip(qCbbtc);
        minOut[2] = _slip(qWeth);
        minOut[3] = _slip(qAero);
        minOut[4] = _slip(qCbeth);

        NARAImmutableBasketPositionManagerV1.BuyParams memory params = NARAImmutableBasketPositionManagerV1.BuyParams({
            paymentToken: USDC,
            inputAmount: input,
        ...
        (uint256 tokenId, uint256[] memory bought) = NARAImmutableBasketPositionManagerV1(mgr).buyBasket(params);
    ...
    function _slip(uint256 q) internal pure returns (uint256) {
        return q * 9950 / 10000;
    }

    function _quoteV3(address i, address o, uint256 a, uint24 f) internal returns (uint256 out) {
        (out,,,) = IQuoterV2(QUOTER).quoteExactInputSingle(IQuoterV2.QuoteExactInputSingleParams(i, o, a, f, 0));
    }
    ...
    function _v3Swap(address adapter, address i, address o, uint256 a, uint256 m, uint24 f)
        internal
        pure
        returns (NARAImmutableBasketPositionManagerV1.SwapInstruction memory)
    {
        return NARAImmutableBasketPositionManagerV1.SwapInstruction({
            adapter: adapter, tokenIn: i, tokenOut: o, amountIn: a, minAmountOut: m, data: abi.encode(f, uint160(0))
        });
    }
